...
public class Nivel1 {
	...
	public void cargarMapa(String archivo) {
		try {
			// --- 1. LIMPIEZA DE MEMORIA (Para borrar el mapa anterior) ---
			for(int i = 0; i < panel.maximoMundoColumna; i++) {
				for(int j = 0; j < panel.maximoMundoFila; j++) {
					mapaNivel1[i][j] = 0; 
				}
			}
			// ------------------------------------------------------------

			// --- 2. LECTURA DEL NUEVO MAPA ---
